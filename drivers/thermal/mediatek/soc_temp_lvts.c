...
/*
 * Vendor glue that lived in <soc/oplus/system/oplus_project.h> and
 * thermal_interface.c/trace.c.  The mainline port has none of those, so
 * provide minimal stubs: these paths only deal with OPPO engineering
 * ("high temp aging") variants and the sysram debug dump.
 */
#define HIGH_TEMP_AGING			1
static int get_eng_version(void) { return 0; }
static void set_cold_interrupt_enable_addr(int enable) { }
static void __iomem *thermal_csram_base;

/*
 * LVTS hardware thermal-reboot protection.  Two gates keep a bad threshold
 * from arming a trip that fires immediately (which resets the SoC with no
 * software trace): one in coefficient space (uncalibrated sensor) and one in
 * raw space (threshold must be clearly colder than what the sensor reads now).
 */
	...
	int trip_point, int tc_id)
{
	 ...
	 * trace. Leave the protection disabled instead.
	 */
	if (coeff_invalid) {
	...
	struct platform_device *pdev)
{
	...
	/*
	 * Arm the hardware thermal-reboot protection now that the coefficients
	 * are known. mainline does not call set_trip_temp() for DT trips at
	 * registration, so this cannot be left to the soc_max critical trip.
	 */
	set_all_tc_hw_reboot(lvts_data);
...
static void lvts_debug_to_sysram(struct lvts_data *lvts_data, int tc_id)
{
	...
	/* sysram dump is a vendor debug-only convenience; no mapping mainline */
...
{
	struct lvts_data *lvts_data = platform_get_drvdata(pdev);
